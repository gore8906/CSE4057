...
def naiveSoftmaxLossAndGradient(
    ...
    ### y_hat - y
    _softmax[outsideWordIdx] = _softmax[outsideWordIdx] - 1
    ### Traspose of outsideVectors = U
    gradCenterVec = np.dot(outsideVectors.T, _softmax) 

    gradOutsideVecs = np.outer(_softmax, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    dot_product = np.dot(outside_vector, centerWordVec)
    loss = -np.log(sigmoid(dot_product))

    gradCenterVec = (sigmoid(dot_product) - 1) * outside_vector 
    gradOutsideVecs = np.zeros_like(outsideVectors) 
    gradOutsideVecs[outsideWordIdx] = (sigmoid(dot_product) - 1) * centerWordVec 

    for negative_idx in indices[1:]: 
        negative_vector = outsideVectors[negative_idx] 
        negative_score = np.dot(negative_vector, centerWordVec)  
        loss -= np.log(sigmoid(-negative_score))  

        gradCenterVec += (1 - sigmoid(-negative_score)) * negative_vector  
        gradOutsideVecs[negative_idx] += (1 - sigmoid(-negative_score)) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside_word in outsideWords:  
        loss_and_gradients = list(word2vecLossAndGradient(
            centerWordVectors[word2Ind[currentCenterWord]], word2Ind[outside_word], outsideVectors, dataset))  
        loss += loss_and_gradients[0]
        gradCenterVecs[word2Ind[currentCenterWord]] += loss_and_gradients[1]
        gradOutsideVectors += loss_and_gradients[2] 
